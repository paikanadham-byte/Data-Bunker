#!/usr/bin/env node
/**
 * Simple Incremental Migration to Neon
 * Migrates data table by table with progress tracking
 */
...
async function run() {
  try {
    ...
    // Companies
    await migrate('companies', [
      'id','company_number','name','legal_name','jurisdiction','company_type',
      'status','incorporation_date','address_line_1','address_line_2','locality',
      'region','postal_code','country','website','phone','email','description',
      'industry','employee_count','annual_revenue','data_source','last_updated','created_at'
    ], 'created_at');
